...
public class Highlights {
        ...
        public static void onLivingRenderPre(RenderLivingEvent.Pre<EntityLivingBase> event) {
            ...
            if (objectMouseOver.typeOfHit == RayTraceResult.Type.ENTITY) {
                ...
                if (objectMouseOver.entityHit == entity) {
                    ...
                    float red = StaticConfig.ENTITY_MODEL_RED;
                    float green = StaticConfig.ENTITY_MODEL_GREEN;
                    float blue = StaticConfig.ENTITY_MODEL_BLUE;
                    float alpha = 1f;
    ...
    public static class BlockAndEntityBoxDrawer {
        ...
        public static void onBlockHighlight(DrawBlockHighlightEvent event) {
            ...
            if ((StaticConfig.RENDER_ENTITY_HITBOX_LINES || StaticConfig.RENDER_ENTITY_HITBOX_OVERLAY) && target.typeOfHit == RayTraceResult.Type.ENTITY) {
                ...
                GlStateManager.enableBlend();
                GlStateManager.tryBlendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
                GlStateManager.glLineWidth(StaticConfig.ENTITY_LINES_WIDTH);
                GlStateManager.disableTexture2D();
                GlStateManager.depthMask(false);

                // Draw boxes/lines/both
                if (StaticConfig.RENDER_ENTITY_HITBOX_OVERLAY) {
                    RenderGlobal.renderFilledBox(
                            ...
                            StaticConfig.ENTITY_OVERLAY_RED, StaticConfig.ENTITY_OVERLAY_GREEN, StaticConfig.ENTITY_OVERLAY_BLUE, StaticConfig.ENTITY_OVERLAY_ALPHA);
                }
                if (StaticConfig.RENDER_ENTITY_HITBOX_LINES) {
                    RenderGlobal.drawSelectionBoundingBox(
                            entityHit.getRenderBoundingBox().offset(-playerXInterp - entityXDiff, -playerYInterp - entityYDiff, -playerZInterp - entityZDiff),
                            StaticConfig.ENTITY_LINES_RED, StaticConfig.ENTITY_LINES_GREEN, StaticConfig.ENTITY_LINES_BLUE, StaticConfig.ENTITY_LINES_ALPHA);
                }
            ...
            // I'm hoping the JVM can optimise at runtime such that the StaticConfig checks no longer exist in the runtime code
            else if ((StaticConfig.RENDER_BLOCK_LINES || StaticConfig.RENDER_BLOCK_OVERLAY || StaticConfig.DISABLE_VANILLA_BLOCK_HIGHLIGHT)
                ...
                if (StaticConfig.DISABLE_VANILLA_BLOCK_HIGHLIGHT) {
                    event.setCanceled(true);
                }

                if (StaticConfig.RENDER_BLOCK_LINES || StaticConfig.RENDER_BLOCK_OVERLAY) {
                    ...
                    if (iblockstate.getMaterial() != Material.AIR && world.getWorldBorder().contains(blockpos)) {
                        ...
                        GlStateManager.enableBlend();
                        GlStateManager.tryBlendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
                        GlStateManager.glLineWidth(StaticConfig.BLOCK_LINES_WIDTH);
                        GlStateManager.disableTexture2D();
                        GlStateManager.depthMask(false);
                        ...
                        if (StaticConfig.RENDER_BLOCK_OVERLAY_USES_COLLISION || StaticConfig.RENDER_BLOCK_LINES_USES_COLLISION) {
                            ...
                            else if (StaticConfig.BLOCK_COLLISION_BOXES_CLAMPED){
...
//                                    AxisAlignedBB resultantBB = tempBB.func_191500_a(offsetFullBlock).expandXyz(0.0020000000949949026D).offset(-d0, -d1, -d2);
                        ...
                        if (StaticConfig.RENDER_BLOCK_OVERLAY) {
                            if (StaticConfig.RENDER_BLOCK_OVERLAY_USES_COLLISION) {
                                for (AxisAlignedBB bb : collisionBoundingBoxes) {
                                    RenderGlobal.renderFilledBox(
                                            bb, StaticConfig.BLOCK_OVERLAY_RED, StaticConfig.BLOCK_OVERLAY_GREEN, StaticConfig.BLOCK_OVERLAY_BLUE, StaticConfig.BLOCK_OVERLAY_ALPHA);
                                }
                            }
                            else {
                                RenderGlobal.renderFilledBox(
                                        selectedBoundingBox, StaticConfig.BLOCK_OVERLAY_RED, StaticConfig.BLOCK_OVERLAY_GREEN, StaticConfig.BLOCK_OVERLAY_BLUE, StaticConfig.BLOCK_OVERLAY_ALPHA);
                            }
                        }
                        if (StaticConfig.RENDER_BLOCK_LINES) {
                            if (StaticConfig.RENDER_BLOCK_LINES_USES_COLLISION) {
                                for (AxisAlignedBB bb : collisionBoundingBoxes) {
                                    RenderGlobal.drawSelectionBoundingBox(
                                            bb, StaticConfig.BLOCK_OVERLAY_RED, StaticConfig.BLOCK_OVERLAY_GREEN, StaticConfig.BLOCK_OVERLAY_BLUE, StaticConfig.BLOCK_OVERLAY_ALPHA);
                                }
                                ...
                                RenderGlobal.drawSelectionBoundingBox(
                                        selectedBoundingBox, StaticConfig.BLOCK_LINES_RED, StaticConfig.BLOCK_LINES_GREEN, StaticConfig.BLOCK_LINES_BLUE, StaticConfig.BLOCK_LINES_ALPHA);
                            }
                        }
